...
Die Hauptherausforderung bei der Simulation bestand darin, die zeitharmonischen 50-Hz-Wechselfelder der \glspl{sammelschiene} darzustellen.
...
Ziel der Messungen ist es zu überprüfen, ob die \glspl{messstromwandler} auch unter dem Einfluss von \glspl{fremdfeld}, wie sie in einem Drehstromsystem durch benachbarte Phasen entstehen, die Anforderungen an die \gls{genauigkeitsklasse} der DIN-Norm 61869-2 einhalten. Die Grenzwerte für die Übersetzungsmessabweichung sind dazu in Tabelle~\ref{tab:grenzwerte_wandler_reduziert} dargestellt.
...
Die durchgeführten Messreihen am \gls{erwaermungspruefstand} sowie die korrespondierenden Simulationen in \gls{femm} liefern ein konsistentes Bild der \gls{fremdfeld}problematik. Die wesentlichen Ergebnisse werden im Folgenden zusammengefasst.

\subsubsection{Validierung des Simulationsmodells}

Zunächst wurde das in Abschnitt \ref{sec:simulation:modellaufbau} beschriebene \gls{femm}-Modell validiert. Hierzu wurden die Simulationsergebnisse für den magnetischen Fluss im Kern des \gls{messstromwandler}s direkt mit den Messwerten vom Prüfstand verglichen.

Es zeigte sich eine hohe Übereinstimmung zwischen der simulierten und der gemessenen \gls{bh-kennlinie}. Insbesondere das Sättigungsverhalten bei hohen Feldstärken sowie der Kniepunkt der Kennlinie wurden vom \gls{quasi-stationaer}en Modell präzise abgebildet. Geringfügige Abweichungen sind primär auf die idealisierte 2D-Annahme der Simulation und leichte Ungenauigkeiten in den Materialdaten der FEMM-Bibliothek zurückzuführen. Das Modell wurde als ausreichend genau für die weitere Untersuchung der Fremdfeldkopplung bewertet.

\subsubsection{Quantifizierung der Fremdfeldeinkopplung}

Die Messungen bestätigten die in der Motivation (Abschnitt \ref{sec:motivation}) formulierte Hypothese: Der \gls{messstromwandler} der mittleren Phase (\acrshort{l2}) ist signifikant von den Magnetfeldern der Außenleiter (\acrshort{l1} und \acrshort{l3}) betroffen.

Bei kompakter Anordnung (z.B. 70 mm \gls{phasenmittelabstand}) führt das \gls{fremdfeld} zu einer Vor-Magnetisierung des Ringkerns von Wandler \acrshort{l2}. Dieser Effekt ist besonders kritisch, da sich der Arbeitspunkt des Wandlers auf der \gls{bh-kennlinie} verschiebt. Der Kern gerät durch die Summe aus Nutzfeld (vom Strom $I_{L2}$) und Störfeld (von $I_{L1}$ und $I_{L3}$) periodisch in die \gls{magnetische-saettigung}.

\subsubsection{Einfluss auf die Genauigkeitsklasse}

Die \gls{magnetische-saettigung} durch das \gls{fremdfeld} wirkt sich direkt auf die Messgenauigkeit aus.
% Hier könnte eine Abbildung eingefügt werden, die den Fehlerverlauf zeigt.
% \einfuegenBildMittel{...}{Beispielhafte Messabweichung für Wandler L2}{fig:beispiel_genauigkeitskurve}
Die Messungen zeigten, dass die \glspl{messstromwandler} bei 100~\% und 120~\% ihres \gls{nennstrom}s die \gls{genauigkeitsklasse} (z.B. Klasse 0,5) oft noch einhalten. Das Problem manifestiert sich jedoch bei kleinen Strömen:

\begin{itemize}
    \item \textbf{Messfehler bei 5~\% \acrshort{in}:} Bei geringen Primärströmen ist das Nutzsignal klein. Das (relativ dazu) starke \gls{fremdfeld} treibt den Kern jedoch bereits in die Nichtlinearität. Dies führte in vielen Messungen bei 5~\% \acrshort{in} zu einer Überschreitung der zulässigen Fehlergrenzen (siehe Tabelle \ref{tab:grenzwerte_wandler_reduziert}). Der Wandler verhält sich nicht mehr linear, und die Messabweichung stieg signifikant an.
\end{itemize}

\subsubsection{Wirkung geometrischer Änderungen}

Die Messungen in "Position 2" (siehe Abbildung \ref{fig:erwaermungspruefstand_wandler_pos2}), bei der die Phase \acrshort{l2} nach vorne verlagert wurde, zeigten eine deutliche Verbesserung. Durch die Erhöhung des Abstands von \acrshort{l2} zu den Störquellen \acrshort{l1} und \acrshort{l3} wird die magnetische Kopplung reduziert.

In dieser Anordnung konnte der Wandler \acrshort{l2} die \gls{genauigkeitsklasse} über den gesamten Messbereich (5~\% bis 120~\% \acrshort{in}) einhalten. Dies belegt, dass die geometrische Anordnung der \glspl{sammelschiene} der entscheidende Faktor zur Minimierung der \gls{fremdfeld}beeinflussung ist.